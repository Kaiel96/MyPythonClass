...
def cast_ray(ray, sphere_list, color, light):
	...
	if visibility > 0 and inter_spheres == []: 
		r += min_tuple[0].color.r*light.color.r*visibility*min_tuple[0].finish.diffuse
		b += min_tuple[0].color.b*light.color.b*visibility*min_tuple[0].finish.diffuse
		g += min_tuple[0].color.g*light.color.g*visibility*min_tuple[0].finish.diffuse
...
def cast_all_rays(min_x, max_x, min_y, max_y, width, height, eye_point, sphere_list, color, light):
	...
	for iy in range(height):
		...
		for ix in range (width):
			...
			print int(min(c.r*255,255)), int (min(c.g*255,255)), int(min(c.b*255,255))
