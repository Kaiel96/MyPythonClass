...
class FinishColors(unittest.TestCase): 

   def test_finish(self):
      f = Finish(.2,4,.4,.04)
      self.assertEqual(.2,f.ambiant)
      self.assertEqual(4,f.diffuse)
      self.assertEqual(.4, f.specular)
      self.assertEqual(.04,f.roughness)
  
   def test_finish_2(self):
      f = Finish(.3,3,.4,.03)
      self.assertEqual(.3,f.ambiant)
      self.assertEqual(3,f.diffuse)
      self.assertEqual(.4, f.specular)
      self.assertEqual(.03,f.roughness)
